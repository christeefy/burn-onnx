...
struct TestData<B: Backend> {
    ...
    softmaxout_1: Param<Tensor<B, 4>>,
}

impl<B: Backend> TestData<B> {
    fn new(device: &B::Device) -> Self {
        // SqueezeNet: input [1, 3, 224, 224], output [1, 1000, 1, 1]
        Self {
            data_0: Initializer::Zeros.init([1, 3, 224, 224], device),
            softmaxout_1: Initializer::Zeros.init([1, 1000, 1, 1], device),
        }
    }
}
...
fn main() {
    ...
    println!("Initializing SqueezeNet 1.0 model...");
    ...
    let data_0 = test_data.data_0.val();
    let data_0_shape: [usize; 4] = data_0.shape().dims();
    println!("  Loaded data_0 with shape: {:?}", data_0_shape);

    // Get the reference output
    let reference_output = test_data.softmaxout_1.val();
    let ref_output_shape: [usize; 4] = reference_output.shape().dims();
    println!(
        "  Loaded reference softmaxout_1 with shape: {:?}",
        ref_output_shape
    );
    ...
    println!("\nWarmup inference...");
    let start = Instant::now();
    let _ = model.forward(data_0.clone());
    ...
    let predicted_output = model.forward(data_0);

    let inference_time = start.elapsed();
    println!("  Inference completed in {:.2?}", inference_time);

    // Display output shape
    let output_shape: [usize; 4] = predicted_output.shape().dims();
    println!("\n  Model output shapes:");
    println!("    softmaxout_1: {:?}", output_shape);

    if output_shape != ref_output_shape {
        eprintln!(
            "FAILED: Expected softmaxout_1 shape {:?}, got {:?}",
            ref_output_shape, output_shape
        );
        std::process::exit(1);
    }
    println!("  Shape matches expected: {:?}", ref_output_shape);

    println!("\nComparing model outputs with reference data...");

    let diff = predicted_output - reference_output;
